...
const calculateDesignArtsTotals = (form, scoreKey = "score") => {
  ...
  const partA = clampScore(
    rowSum("lectures", 40) + rowSum("courseFile", 20) + (scoreKey === "score" ? innovativeTeachingScore(form.innovDetails, form.innovScore, 10) : clampScore(form[scoreKeyForInnov(scoreKey)], 10)) +
              ...
              {mode === "self" ? (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {INNOVATIVE_METHODS.map((method) => {
                    ...
                    return (
                      <button key={method} type="button" disabled={!editableSelf} onClick={() => toggleMethod(method)} style={{ border: selected ? "1px solid #be185d" : "1px solid #cbd5e1", background: selected ? "#fdf2f8" : "#fff", color: selected ? "#9d174d" : "#334155", borderRadius: 5, padding: "5px 7px", fontSize: 10, fontWeight: 800, cursor: editableSelf ? "pointer" : "not-allowed" }}>
                        {method}
                      </button>
                    );
                  })}
